...
@Service @Slf4j public class LocalFileStore implements com.vjoon.se.core.services.FileStore {

    private static final String CAMLE_BASE = "assets/";
    @Value("${base.path}") @Setter private String basePath;

    @EndpointInject(uri = "direct:fileStore") private ProducerTemplate template;

    @Override
    public void save(@NotNull String nameSpace, @NotNull String key, @NotNull InputStream stream, boolean overwrite) {
    ...
    String createFileNameFromID(String nameSpace, String key) {
        ...
        return basePath + File.separator + nameSpace + File.separator + Arrays.stream(parts)
                .collect(Collectors.joining(File.separator)) + File.separator + mediaID;
    }

    @Override public InputStream getStream(String nameSpace, String key) {
        try {
    ...
    @Override public String getHash(String nameSpace, String key) {
